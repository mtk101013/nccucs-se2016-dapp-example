...
contract Congress is owned, tokenRecipient {
    ...
    event Voted(uint candidateID, bool position, address voter, string justification);
    ...
    /* First time setup */
    function Congress(
        uint minimumQuorumForCandidates,
...
function getSender() returns(address){
        ...
        returns (uint candidateID)
    {
        ...
        c.numberOfVotes = 0;
        CandidateAdded(candidateID, candAddr, etherAmount, candName);
        ...
        returns (uint voteID)
    {
        ...
        //if (supportsCandidate) {                         // If they support the candidate
            c.currentResult++;                          // Increase score
        /*} else {                                        // If they don't
            c.currentResult--;                          // Decrease the score
        }*/
        // Create a log of this event
        Voted(candidateNumber,  supportsCandidate, msg.sender, justificationText);
    }
    ...
    function executeCandidate(uint candidateNumber, bytes transactionBytecode) returns (int result) {
        Candidate c = candidates[candidateNumber];
        ...
        */
        ...
        if (now < c.votingDeadline
            || c.executed
            || c.candidateHash != sha3(c.recipient, c.amount, transactionBytecode)
            || c.numberOfVotes < minimumQuorum)
            throw;
        ...
        if (c.currentResult > majorityMargin) {
            ...
            c.executed = true;
            if (!c.recipient.call.value(c.amount * 1 ether)(transactionBytecode)) {
                throw;
            }
